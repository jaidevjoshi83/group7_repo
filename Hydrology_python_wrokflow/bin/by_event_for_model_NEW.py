...
# get_ipython().magic(u'matplotlib inline') # changed
import matplotlib #changed
matplotlib.use('Agg') #changed 
import os
import sys
...
parser.add_argument("-i", "--in_file_1",
                    required=True,
                    default=None,
                    help="Flood event file")


parser.add_argument("-I", "--in_file_2",
                    required=True,
                    default=None,
                    help="daily observations file")
                    
parser.add_argument("-O", "--out_file",
                    required=False,
                    default='for_model_avgs.csv',
                    help="out_file")
...
first_input = args.in_file_1
second_input = args.in_file_2
...
flood_events = pd.read_csv(first_input)
...
# jqn = flood_df[flood_df['event'].str.contains('Joaquin')] # changed


# In[12]:

# jqn[jqn['location'].duplicated(keep=False)] # changed
...
# flood_df[flood_df['event'].str.contains('2/25/2016')] # changed
...
feature_df = pd.read_csv(second_input)
...
# event_df_for_storage.to_csv('{}event_data.csv'.format(data_dir), index=False) # changed
# event_df_for_storage.to_sql(name='event_data', con=con, if_exists='replace', index=False) # changed
...
avdf.to_csv(os.path.join(out_dir,out_csv))
