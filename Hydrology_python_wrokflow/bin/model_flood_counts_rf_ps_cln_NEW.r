...
#clear_db_tables = function(models, con, suffix){    # changed
...
suffix = 'out'
# clear_db_tables(model_types, con, suffix) # changed
...
for (i in 1:101){
  ...
  for (model in model_types){
    ...
    model_results = run_model(model, train_data, train_in_data, train_out_data, test_in_data, test_out_data, fomla)
	  ...
	  write.table(all_trn_df, paste(options[2],'/',model, '_', suffix, '_train.csv', sep=""), append=TRUE,  sep=",", col.names = F)
	  write.table(all_tst_df, paste(options[2],'/',model, '_', suffix, '_test.csv', sep=""), append=TRUE,  sep=",", col.names = F)
...
write.csv(import_df, paste(options[2],'/rf_impo_', suffix, sep=""), append=TRUE)
